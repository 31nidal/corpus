...
test('couverture organisée : cours, exercices, schémas et repères 3D valides',()=>{
 ...
 for(const c of courses){
  ...
  expect(c.sections.length,c.id).toBeGreaterThanOrEqual(5)
  expect(c.sections.length,c.id).toBeLessThanOrEqual(8)
  if(c.glossary)expect(c.glossary.length,c.id).toBeGreaterThanOrEqual(2)
 ...
 const groupedIds=groupCourses(courses).flatMap(s=>s.groups.flatMap(g=>g.courses.map(c=>c.id)))
 expect(groupedIds).toHaveLength(courses.length)
 expect(new Set(groupedIds)).toEqual(new Set(courses.map(c=>c.id)))
...
test('matière → région → chapitre, recherche sans accents et retour navigateur',async({page})=>{
 ...
 await page.locator('.subject-card.anatomy-subject').click()
...
test('mobile : matières séparées et quiz filtré, sans chargement 3D',async({page})=>{
 ...
 await expect(page.locator('.question-kicker .study-eyebrow')).toHaveText('Histologie')
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(393)
