...
export default function App() {
  ...
  const [atlas3DModalOpen, setAtlas3DModalOpen] = useState(false)
  const [capturedScene, setCapturedScene] = useState<Atlas3DScene | null>(null)

  const openAtlas3DModal = () => {
    if (!selected) return
    if (!manifest?.atlasRevision) return
    const cameraPose = apiRef.current?.capture() || { position: [0, 0.04, 6.1], target: [0, 0, 0] }
    const sceneToCapture: Atlas3DScene = {
      modelKey: body === 'female' ? 'female_detail' : detailMode ? 'bp3d_detail' : 'bp3d_overview',
      atlasRevision: manifest.atlasRevision,
      camera: {
      ...
      opacity: { ...opacity },
      cut: {
        enabled: Boolean(cut.enabled),
        axis: cut.axis,
        position: cut.position,
        flipped: Boolean(cut.flipped),
        guide: Boolean(cut.guide),
      },
      isolationStructureId: isolated && selectedId ? selectedId : null,
      hiddenStructureIds: [...hiddenIds],
    }
    setCapturedScene(sceneToCapture)
    setAtlas3DModalOpen(true)
  }
  ...
  const courseForSelectedStructure=()=>{
    ...
    const exact=courses.find(course=>course.id===selected.id&&course.category==='Anatomie')
    if(exact)return exact.id
    const anchored=courses.find(course=>course.structure===selected.id&&course.category==='Anatomie')
    if(anchored)return anchored.id
    const related=structureCourseCandidates(selected).find(id=>courses.some(course=>course.id===id))
    return related??lessonFor(selected)?.id??null
  }
  ...
  return <main className="experience" data-body={body} data-workspace={learningOpen?'courses':practiceOpen?'practice':myCoursesOpen?'my-courses':flashcardsOpen?'flashcards':'atlas'} data-chat={chatOpen} data-selected={selectedId ?? ''} data-loaded={isLoaded}>
    ...
    {atlas3DModalOpen && capturedScene && manifest && manifest.atlasRevision && selected && (
      <Atlas3DNoteModal
        isOpen={atlas3DModalOpen}
        onClose={() => setAtlas3DModalOpen(false)}
        modelKey={body === 'female' ? 'female_detail' : detailMode ? 'bp3d_detail' : 'bp3d_overview'}
        atlasRevision={manifest.atlasRevision}
        scene={capturedScene}
        initialStructure={{ id: selected.id, name: description?.name || selected.name }}
        manifest={manifest}
        onCreated={() => {
          setAtlas3DModalOpen(false)
        }}
      />
    )}
  </main>
