...
function readRoute() {
  ...
  return { id: params.get('structure'), detail: params.get('mode') !== 'overview', view:decodeView(params.get('view')) }
}
...
export default function App() {
  ...
  const writeRoute = (id: string | null, detail = detailMode) => {
  ...
  useEffect(() => {
    ...
    setSelectedId(structure?.id ?? null); setIsolated(route.view?.isolated ?? (pendingIsolation.current !== null && pendingIsolation.current === structure?.id)); pendingIsolation.current=null; setHover(null)
    if(route.view){
  ...
  const reset = () => { setAnimation(null);setCameraRestore(null);setCut(defaultCut); setGuided(false); setSelectedId(null); setIsolated(false); setHover(null); setResetKey(k => k + 1); setOrientation('front'); writeRoute(null) }
  ...
  const visibleCount = structures.filter(s => !s.aggregate && visibility[s.group] && opacity[s.group]>0 && load.ready.includes(s.group) && s.meshNames.some(name => !hiddenMeshes.has(name) && (!isolated || !selected || selected.meshNames.includes(name)))).length
  ...
  const visitStep = (index: number) => {
    ...
    writeRoute(tourSteps[index].id, true)
  }
  const prepareQuestion = (index:number) => {
    setDetailMode(true);setCameraRestore(null);writeRoute(null,true);setGuided(false);setSelectedId(null);setIsolated(false);setHiddenIds([])
  ...
  const openLearning=()=>openStudy('cours',lessonFor(selected)?.id??null)
  const openPractice=()=>openStudy('entrainement')
  ...
  return <main className="experience" data-workspace={learningOpen?'courses':practiceOpen?'practice':'atlas'} data-chat={chatOpen} data-selected={selectedId ?? ''} data-loaded={isLoaded}>
    ...
    {!quiz && <ChatAssistant open={chatOpen} selectedId={selectedId} name={description?.name??'Le corps humain'} system={description?.system??''} lesson={learningOpen?currentLesson:lessonFor(selected)?.id??''} level={level} onAction={executeAction} onOpen={setChatOpen}/>}
    ...
    <footer className="bottombar"><div className="model-status"><span className={`status-dot ${load.complete ? 'is-ready' : ''}`} /><span>{load.complete ? `${visibleCount} structures actives` : 'Préparation de l’exploration'}</span><span className="footer-divider">/</span><span>BodyParts3D</span></div><button className="education-note" onClick={()=>{setSelectedId(null);setIsolated(false);setCatalogOpen(v=>!v)}}>Index des structures <ArrowUpRight size={12}/></button><div className="footer-actions"><button onClick={() => setModal('about')}>Sources & crédits <ArrowUpRight size={12} /></button><button onClick={() => setModal('help')} aria-label="Aide à la navigation"><CircleHelp size={17} /></button></div></footer>

    </div>
    {learningOpen&&<CoursesWorkspace initial={courseToOpen} completed={completed} complete={completeCourse} explore={id=>{setLearningOpen(false);selectStructure(id)}} practice={id=>openStudy('entrainement',id)} navigate={id=>openStudy('cours',id)}/>}
    {practiceOpen&&<PracticeWorkspace course={practiceCourse} learn={id=>openStudy('cours',id)} start3D={startQuiz}/>}
