...
import{BarChart3,BookOpen,Brain,Check,ChevronRight,Copy,Download,Image as ImageIcon,Layers,Plus,RotateCcw,Search,Sparkles,Trash2,X}from'lucide-react'
...
import type{Flashcard,FlashcardDeck,FlashcardPreview,FlashcardStats,GenerationSource,ImageOcclusionMask,NoteType}from'./flashcardsTypes'
import GenerationDialog from'./GenerationDialog'
import{ReviewImageOcclusion}from'./ReviewImageOcclusion'
import{ReviewAtlas3D}from'./ReviewAtlas3D'
...
export default function FlashcardsWorkspace({openCourse,openDocument}:{openCourse:(courseId:string,sectionId?:string|null)=>void;openDocument:(documentId:string,sectionId?:string|null)=>void}){
 ...
 const [imageOcclusionState, setImageOcclusionState] = useState<{
   imageSrc: string
   imageBlob?: Blob | null
 ...
 useEffect(()=>{setCards([]);setDecks([]);setStats(null);setReview(null);setAtlasReady(false);setCardPreview(null);setPreviewLoading(false);setPreviewError('');setEditor(null);setGeneration(null);setFreeText('');setTypedInput('')},[account.user?.id])
 const startReview=(deckId?:string)=>perform(async()=>{const due=await reviewQueue(deckId);setReview(due);setReviewIndex(0);setRevealed(false);setAtlasReady(false);setCardPreview(null);setPreviewLoading(false);setPreviewError('');setReviewStarted(Date.now());setTypedInput('')})
 const reveal=async()=>{const c=review?.[reviewIndex];if(!c)return;if(c.cardType==='atlas_3d'&&!atlasReady)return;setRevealed(true);setPreviewLoading(true);setPreviewError('');setCardPreview(null);try{const p=await previewCard(c.id);setCardPreview(p.preview)}catch(e){setPreviewError((e as Error).message||'Impossible de calculer les prochains rappels.')}finally{setPreviewLoading(false)}}
 const rate=(rating:'again'|'hard'|'good'|'easy')=>perform(async()=>{const card=review?.[reviewIndex];if(!card||!revealed||!cardPreview||previewLoading)return;if(card.cardType==='atlas_3d'&&!atlasReady)return;await reviewCard(card.id,rating,Date.now()-reviewStarted,card.review?.dueAt,card.review?.reviewVersion,cardPreview.id);
  ...
  setReviewIndex(i=>i+1);setRevealed(false);setAtlasReady(false);setCardPreview(null);setPreviewLoading(false);setPreviewError('');setReviewStarted(Date.now());setTypedInput('')})

 const openEditCard = async (card: Flashcard) => {
   if (card.cardType === 'atlas_3d') {
     window.location.hash = '#tab=atlas'
     return
   }
   if (card.noteId) {
     try {
       const note = await fetchNote(card.noteId)
       if (note.noteType === 'atlas_3d') {
         window.location.hash = '#tab=atlas'
         return
       }
       if (note.noteType === 'image_occlusion' && note.assetId) {
         setImageOcclusionState({
           imageSrc: `/api/flashcards/assets/${encodeURIComponent(note.assetId)}`,
           initialAssetId: note.assetId,
           imageBlob: null,
           noteId: note.id,
           noteVersion: note.noteVersion,
           initialMasks: (note.fields?.masks as ImageOcclusionMask[]) || [],
           initialPrompt: (note.fields?.prompt as string) || '',
           initialExtra: (note.fields?.extra as string) || '',
           initialSubject: note.subject || '',
           initialChapter: note.chapter || '',
           initialTags: note.tags || [],
           sourceMetadata: note.source?.type === 'study_document' ? {
             sourceKind: 'study_document',
             sourceDocumentId: note.source.documentId,
             sourcePage: note.source.locator?.page,
           } : null,
         })
         return
       }
 ...
 if(review)return <section className="study-workspace flash-workspace flash-review"><header><button className="study-text-link" disabled={pending} onClick={()=>{setReview(null);setCardPreview(null);setPreviewLoading(false);setPreviewError('');void refresh()}}><X size={16}/>Quitter</button><span>{Math.min(reviewIndex+1,review.length)} / {review.length}</span></header>{error&&<p className="flash-error" role="alert">{error}</p>}{current?<article className="review-card"><span className="study-eyebrow"><span>{decks.find(d=>d.id===current.deckId)?.name||'Flashcard'}</span>{current.cardType==='cloze'?<> · <span>Texte à trous</span></>:current.cardType==='typed'?<> · <span>Réponse saisie</span></>:current.cardType==='image_occlusion'?<> · <span>Image Occlusion</span></>:current.cardType==='atlas_3d'?<> · <span>Atlas 3D</span></>:null}</span>
  <div className="review-face">
    <small>RECTO</small>
    <h1>{current.front}</h1>
    {current.cardType==='image_occlusion'&&<ReviewImageOcclusion card={current} isFlipped={revealed}/>}
    {current.cardType==='atlas_3d'&&<ReviewAtlas3D card={current} isFlipped={revealed} onError={msg=>setPreviewError(msg)} onReadyChange={setAtlasReady}/>}
  ...
  {revealed?<><div className="review-answer"><small>VERSO</small>
    ...
    {current.cardType==='cloze'?<p dangerouslySetInnerHTML={{__html:current.back}}/>:<p className={current.cardType==='image_occlusion'||current.cardType==='atlas_3d'?'text-lg font-semibold text-emerald-400':''}>{current.back}</p>}
  </div>
  {previewLoading?<div><p className="review-loading-status" aria-live="polite">Calcul des prochains rappels…</p><div className="review-ratings"><button disabled><strong>À revoir</strong><small>…</small></button><button disabled><strong>Difficile</strong><small>…</small></button><button disabled><strong>Correct</strong><small>…</small></button><button disabled><strong>Facile</strong><small>…</small></button></div></div>:previewError?<div className="review-preview-error" role="alert"><p className="flash-error">{previewError}</p><button className="study-secondary" onClick={reveal}>Réessayer</button></div>:cardPreview?<div className="review-ratings"><button disabled={pending || (current?.cardType === 'atlas_3d' && !atlasReady)} onClick={()=>rate('again')}><strong>À revoir</strong><small>{cardPreview.labels.again}</small></button><button disabled={pending || (current?.cardType === 'atlas_3d' && !atlasReady)} onClick={()=>rate('hard')}><strong>Difficile</strong><small>{cardPreview.labels.hard}</small></button><button disabled={pending || (current?.cardType === 'atlas_3d' && !atlasReady)} onClick={()=>rate('good')}><strong>Correct</strong><small>{cardPreview.labels.good}</small></button><button disabled={pending || (current?.cardType === 'atlas_3d' && !atlasReady)} onClick={()=>rate('easy')}><strong>Facile</strong><small>{cardPreview.labels.easy}</small></button></div>:null}</>:<button className="study-primary" disabled={current?.cardType === 'atlas_3d' && !atlasReady} onClick={reveal}>Afficher la réponse</button>}</article>:<div className="flash-empty"><h2>Session terminée</h2><p>{review.length} carte(s) révisée(s). Les cartes « À revoir » seront de nouveau disponibles dans environ dix minutes.</p><button disabled={pending} onClick={()=>{setReview(null);setCardPreview(null);setPreviewLoading(false);setPreviewError('');void refresh()}}>Retour aux flashcards</button></div>}</section>
 ...
 view==='decks'?<div><div className="flash-create-deck"><input maxLength={100} aria-label="Nom du nouveau deck" placeholder="Nom du nouveau deck" value={newDeck} onChange={e=>setNewDeck(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void addDeck()}}/><button className="study-primary" disabled={pending||!newDeck.trim()} onClick={addDeck}><Plus size={16}/>Créer un deck</button></div><div className="deck-grid">{decks.map(deck=><article key={deck.id}><span>{deck.subject||'Deck personnel'}</span><h3>{deck.name}</h3><p>{deck.cardCount} cartes · {deck.dueCount} à revoir</p><div><button onClick={()=>{const name=prompt('Nouveau nom du deck',deck.name);if(name)void perform(async()=>{await updateDeck(deck.id,{name});await refresh()})}}>Renommer</button><button onClick={()=>{if(confirm(`Supprimer « ${deck.name} » et ses cartes ?`))void perform(async()=>{await deleteDeck(deck.id);if(deckFilter===deck.id)setDeckFilter('');await refresh()})}}><Trash2 size={14}/>Supprimer</button></div></article>)}</div></div>:
   view==='cards'?<div><div className="flash-card-toolbar"><div><Search size={16}/><input aria-label="Rechercher dans les flashcards" placeholder="Rechercher recto, verso, matière, tag…" value={query} onChange={e=>setQuery(e.target.value)}/></div><select aria-label="Filtrer par deck" value={deckFilter} onChange={e=>setDeckFilter(e.target.value)}><option value="">Tous les decks</option>{decks.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select><input aria-label="Filtrer par matière" placeholder="Matière" value={subjectFilter} onChange={e=>setSubjectFilter(e.target.value)}/><input aria-label="Filtrer par chapitre" placeholder="Chapitre" value={chapterFilter} onChange={e=>setChapterFilter(e.target.value)}/><input aria-label="Filtrer par tag" placeholder="Tag" value={tagFilter} onChange={e=>setTagFilter(e.target.value)}/><button className="study-primary" disabled={!decks.length||pending} onClick={()=>setEditor({...emptyEditor,deckId:deckFilter||decks[0].id})}><Plus size={16}/>Nouvelle carte</button><button className="study-secondary" disabled={!decks.length||pending} onClick={()=>imageFileInputRef.current?.click()} title="Créer une flashcard visuelle"><ImageIcon size={16}/>Image Occlusion</button><input ref={imageFileInputRef} type="file" accept="image/png,image/jpeg,image/webp" style={{display:'none'}} onChange={e=>{const file=e.target.files?.[0];if(!file)return;if(!['image/png','image/jpeg','image/webp'].includes(file.type)){setError('Format non supporté. Formats acceptés : PNG, JPEG, WebP.');return}const previewUrl=URL.createObjectURL(file);setImageOcclusionState({imageSrc:previewUrl,imageBlob:file,sourceMetadata:{sourceKind:'upload'}});e.target.value=''}}/></div><div className="flash-card-list">{cards.map(card=><article key={card.id}><div><span>{decks.find(d=>d.id===card.deckId)?.name} · {card.subject||'Sans matière'}<span className="card-type-badge">{card.cardType==='image_occlusion'?'Image Occlusion':card.cardType==='atlas_3d'?'Atlas 3D':card.cardType==='cloze'?`Trou (${card.derivationKey})`:card.cardType==='typed'?'Réponse saisie':card.derivationKey==='reverse'?'Inversée':card.derivationKey==='forward'?'Directe':'Standard'}</span></span><h3>{card.front}</h3><p>{card.back}</p>{card.tags.length?<div className="flash-tags">{card.tags.map(tag=><em key={tag}>{tag}</em>)}</div>:null}</div><div className="flash-card-actions">{(card.source.courseId||card.source.documentId)&&<button onClick={()=>sourceAction(card)}>Voir dans {card.source.documentId?'le PDF':'le cours'}</button>}{card.cardType==='atlas_3d'?<button title="Ouvrir dans l’Atlas 3D" onClick={()=>{window.location.hash='#tab=atlas'}}>Ouvrir l’Atlas</button>:<button onClick={()=>void openEditCard(card)}>Modifier</button>}<select aria-label={'Déplacer '+card.front} value={card.deckId} onChange={e=>void perform(async()=>{await moveCard(card.id,e.target.value);await refresh()})}>{decks.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}</select><button title="Dupliquer" onClick={()=>void perform(async()=>{await duplicateCard(card.id);await refresh()})}><Copy size={15}/></button><button title="Supprimer" onClick={()=>confirm('Supprimer cette carte et son historique de révision ?')&&void perform(async()=>{await deleteCard(card.id, typeof card.noteVersion === 'number' ? card.noteVersion : undefined);await refresh()})}><Trash2 size={15}/></button></div></article>)}</div>{nextCursor&&<button className="study-secondary flash-load-more" disabled={pending} onClick={loadMore}>Charger davantage de cartes</button>}{!cards.length&&<p className="flash-empty">Aucune carte ne correspond à ces filtres.</p>}</div>:
 ...
 {generation&&<GenerationDialog source={generation} onClose={()=>setGeneration(null)} onSaved={info=>{setFreeText('');void refresh();if(info)setToast(`${info.count} carte${info.count>1?'s':''} enregistrée${info.count>1?'s':''} dans le deck « ${info.deckName} »`)}}/>}
  {exportOpen&&<div className="flash-modal-backdrop"><section className="flash-modal flash-card-editor" role="dialog" aria-modal="true"><header><h2>Exporter vers Anki</h2><button className="icon-button" onClick={()=>setExportOpen(false)}><X/></button></header>{cards.some(c=>c.cardType==='image_occlusion'||c.cardType==='atlas_3d')&&<div className="flash-warning-banner" style={{background:'rgba(245,158,11,0.1)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:'8px',padding:'0.75rem 1rem',marginBottom:'1rem',color:'#fef3c7',fontSize:'0.85rem'}}><strong>Information sur les cartes visuelles et 3D :</strong> Les flashcards d’occlusion d’image et d’Atlas 3D sont exportées au format texte simplifié (recto avec consigne et verso avec étiquette). Pour l’expérience interactive complète (masques visuels ou scène 3D navigable), révisez directement dans MyCorpus.</div>}<label>Contenu<select value={exportScope} onChange={e=>setExportScope(e.target.value as typeof exportScope)}><option value="all">Toutes mes flashcards</option><option value="due">Uniquement les cartes à réviser</option><option value="decks">Un ou plusieurs decks</option><option value="course">Les cartes d’un cours</option></select></label>{exportScope==='decks'&&<fieldset>{decks.map(deck=><label className="flash-check" key={deck.id}><input type="checkbox" checked={exportDecks.includes(deck.id)} onChange={e=>setExportDecks(ids=>e.target.checked?[...ids,deck.id]:ids.filter(id=>id!==deck.id))}/>{deck.name}</label>)}</fieldset>}{exportScope==='course'&&<label>Cours<select value={exportCourse} onChange={e=>setExportCourse(e.target.value)}><option value="">Choisir un cours</option>{stats?.courses.map(course=><option key={course.id} value={course.id}>{course.title||course.id} · {course.total} cartes</option>)}</select></label>}<button className="study-primary" disabled={exportScope==='decks'&&!exportDecks.length||exportScope==='course'&&!exportCourse} onClick={async()=>{try{const filters=exportScope==='due'?{due:true}:exportScope==='decks'?{deckIds:exportDecks}:exportScope==='course'?{courseId:exportCourse}:{};downloadCsv(await exportAnki(filters),'mycorpus-flashcards.csv');setExportOpen(false)}catch(e){setError((e as Error).message)}}}><Download size={16}/>Télécharger le fichier Anki</button></section></div>}
 {imageOcclusionState&&(
   <ImageOcclusionEditor
     imageSrc={imageOcclusionState.imageSrc}
     initialAssetId={imageOcclusionState.initialAssetId}
     imageBlob={imageOcclusionState.imageBlob}
     sourceMetadata={imageOcclusionState.sourceMetadata}
     defaultDeckId={deckFilter||decks[0]?.id||''}
     decks={decks}
     noteId={imageOcclusionState.noteId}
     noteVersion={imageOcclusionState.noteVersion}
     initialMasks={imageOcclusionState.initialMasks}
     initialPrompt={imageOcclusionState.initialPrompt}
     initialExtra={imageOcclusionState.initialExtra}
     initialSubject={imageOcclusionState.initialSubject}
     initialChapter={imageOcclusionState.initialChapter}
     initialTags={imageOcclusionState.initialTags}
     onCreated={note=>{
       setImageOcclusionState(null)
       void refresh()
       setToast(`Note d’occlusion créée (${note.cards?.length||1} carte(s))`)
     }}
     onUpdated={()=>{
       setImageOcclusionState(null)
       void refresh()
       setToast('Note d’occlusion mise à jour')
     }}
     onCancel={()=>setImageOcclusionState(null)}
   />
 )}
