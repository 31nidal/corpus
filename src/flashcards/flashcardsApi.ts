...
export async function generateDrafts(source:GenerationSource,level:'essential'|'standard'|'complete',count:number,sectionIds:string[],pages?:{startPage?:number;endPage?:number}){
 ...
 const payload={text,qcm:source.qcm,segments:selected.map(section=>({id:section.id,title:section.title,text:section.text||'',startPage:section.startPage,endPage:section.endPage})),level,count,courseId:source.courseId,documentId:source.documentId,subject:source.subject,chapter:source.chapter||source.title,sectionIds,sectionId:source.sectionId||(selected.length===1?selected[0].id:undefined),locator:source.locator,...pages}
 return request<{drafts:FlashcardDraft[];generation:{mode:string;produced:number;persisted:boolean}}>(`generate/${source.kind}`,{method:'POST',body:JSON.stringify(payload)})
}
export async function exportAnki(filters:Record<string,unknown>={}){const response=await fetch('/api/flashcards/export',{method:'POST',credentials:'same-origin',headers,body:JSON.stringify(filters)});if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.error||'Export impossible.')}return response.text()}
export function downloadCsv(content:string,name='mycorpus-flashcards.csv'){const blob=new Blob([content],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=name;link.click();URL.revokeObjectURL(url)}
