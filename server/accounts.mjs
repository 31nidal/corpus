...
function mergeValue(key, value, before, current) {
  ...
  if (key === 'corpus-practice-v1') {
    ...
    for (const [id, result] of Object.entries(incoming)) {
      ...
      if (delta > 0) {
        const currentRecord = saved[id] || {}
        const newest = (result.lastReviewed ?? 0) >= (currentRecord.lastReviewed ?? 0) ? result : currentRecord
        saved[id] = {seen: (currentRecord.seen || 0) + delta, correct: (currentRecord.correct || 0) + Math.max(0, result.correct - oldResult.correct), wrong: newest.wrong,
          streak: newest.streak ?? currentRecord.streak, interval: newest.interval ?? currentRecord.interval, due: newest.due ?? currentRecord.due, lastReviewed: newest.lastReviewed ?? currentRecord.lastReviewed}
      }
    }
