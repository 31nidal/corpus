...
## Flashcards et Notes pédagogiques

La génération locale ou assistée produit des brouillons de **Notes pédagogiques** structurées (`basic`, `reverse`, `bidirectional`, `cloze`, `typed`). Les générations depuis les cours, Study, les erreurs QCM ou du texte libre ne persistent rien immédiatement : l'utilisateur prévisualise et valide les cartes dérivées avant enregistrement via `POST /api/flashcards/notes/bulk`.

Lors de la génération (`POST /api/flashcards/generate/*`), le serveur émet un identifiant opaque (`generationId`) associé à un reçu temporaire stocké dans `flashcard_generation_receipts` (TTL de 2 heures, purgé automatiquement au démarrage, lors des générations et des sauvegardes). Pour Study, la provenance est vérifiée directement contre les tables SQLite `study_documents` et `study_sections`. Pour les cours du catalogue et les corrections QCM, le reçu garantit que les notes sont strictement dérivées du texte soumis lors de la génération (les assertions médicales `back`, `answer`, `text`, trous cloze et `sourceExcerpt` sont vérifiées de manière déterministe avant toute persistance). Ce reçu constitue la garantie de provenance actuelle, en attendant un référentiel canonique complet du catalogue hébergé côté serveur.

Le binding entre le reçu et les notes enregistrées est strict :
- Si `generationId` est fourni, chaque note du lot doit être liée au reçu : son type de source doit correspondre exactement au type du reçu, sans rétrogradation possible vers `manual` ou `free_text`, et avec conservation exacte des identifiants (`courseId`, `documentId`, `sectionId`).
- Si une note possède une source générée (`catalog_course`, `study_document`, `qcm_error`, `free_text`), la présence d'un `generationId` valide est obligatoire.
- Pour `free_text`, la citation source (`source_excerpt`) est systématiquement forcée à `null` en base.
- La normalisation déterministe (`normalize` et `normalizeTypedAnswer`) préserve intégralement les accents français (`NFKC`/`NFC` sans suppression diacritique).

Chaque Note enregistrée projette automatiquement une ou plusieurs cartes dérivées (`Flashcard`), chacune disposant de son propre état et historique de planification FSRS indépendant. L'enregistrement par lot accepte un `requestId` stable géré sous transaction immédiate SQLite (`BEGIN IMMEDIATE`) avec `PRAGMA busy_timeout=5000` : des requêtes concurrentes ou des reprises réseau à contenu identique retournent le résultat déjà créé sans générer de doublons ni d'erreur de contention. Les révisions concurrentes sont protégées par `expectedDueAt`. L’export applique les filtres avant pagination et refuse les sélections de plus de 10 000 cartes.

Les tables `flashcard_decks`, `flashcard_notes`, `flashcards`, `flashcard_reviews`, `flashcard_review_logs`, `flashcard_generation_receipts` et `flashcard_save_requests` sont isolées par compte utilisateur et nettoyées lors de la suppression de compte. Les références visuelles sont prévues dans `visual_json`.
