...
export class FlashcardRepository {
  ...
  listCards(userId, filters = {}) {
    ...
    if (filters.query) {
      where.push('(LOWER(c.front) LIKE ? OR LOWER(c.back) LIKE ? OR LOWER(c.subject) LIKE ? OR LOWER(c.chapter) LIKE ? OR LOWER(c.tags_json) LIKE ?)')
  ...
  createCard(userId, value) {
    ...
    this.db.prepare(`
      INSERT INTO flashcards(
        id, user_id, deck_id, front, back, subject, chapter, tags_json, visual_json,
    ...
    `).run(
      id, userId, value.deckId, value.front, value.back, value.subject || null, value.chapter || null,
